...
class SettingsController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        if(isset($image)){
            ...
            if(!Storage::disk('public')->exists('profile/admin'))
            {
                Storage::disk('public')->makeDirectory('profile/admin');
            }
            //delete old image
            if(Storage::disk('public')->exists('profile/admin/'.$user->image))
            {
                Storage::disk('public')->delete('profile/admin/'.$user->image);
            }
            //resize image for category and upload
            $profileAdmin = Image::make($image)->resize(500,480)->save();
            Storage::disk('public')->put('profile/admin/'.$imageName,$profileAdmin);
        }
        else{
